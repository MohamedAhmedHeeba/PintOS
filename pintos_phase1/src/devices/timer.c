...
bool compare_wakeup(const struct list_elem *a_, const struct list_elem *b_, void *aux UNUSED);

/*sleeping list add sleeping threads in it*/
...
timer_sleep (int64_t ticks)
{
    ...
    // insert the current thread in thr sleeping list
    list_insert_ordered(&sleeping_list,&t->elem,compare_wakeup,NULL);
...
timer_interrupt (struct intr_frame *args UNUSED)
{
    ...
    if(thread_mlfqs)
    {
        //increase recent CPU for the cirrent thread by 1
...
bool
compare_wakeup(const struct list_elem *a_, const struct list_elem *b_, void *aux UNUSED)
{
